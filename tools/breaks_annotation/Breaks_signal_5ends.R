#############################################################################
##
## What: Breaks_signal_5ends.R
## Who: Giuseppe Petrosino
## When: 2024-05-03
##
## Script to count the number of reads for the breaks on the 5' ends,
...
file_a <- parseArgs(args,"ExpectedBreaks5End=") 
file_b <- parseArgs(args,"Bed=") 
qPCR <- parseArgs(args,"qPCR=")
out <- parseArgs(args,"out=")

runstr <- paste0("Call with: Rscript Breaks_signal_5ends.R [ExpectedBreaks5End=",file_a,"] [Bed=",file_b,"] [qPCR=",qPCR,"] [out=",out,"]")
cat(runstr)
