Cutadapt_3ends = {
    ...
    def SAMPLENAME_BASE_PRUNED = SAMPLENAME_BASE.replace(".R1.m.end3", "") // delete .R1 in combined log file of pe design
	...
	transform("*.fastq.gz") to (".trimmed.fastq.gz") {
		exec """
            ...
            module load cutadapt/${CUTADAPT_VERSION} &&
                        ...
                        cutadapt $CUTADAPT_THREADS $CUTADAPT_FLAGS --output=\${TMPDIR}/${SAMPLENAME_BASE_PRUNED}.R1.m.end3.trimmed.fastq.gz --paired-output \${TMPDIR}/${SAMPLENAME_BASE_PRUNED}.R2.m.end3.trimmed.fastq.gz $input1 $input2 1> ${TRIM_LOGS}/${SAMPLENAME_BASE_PRUNED}.end3.cutadapt.log &&
